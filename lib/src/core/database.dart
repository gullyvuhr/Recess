...
class RecessDatabase extends GeneratedDatabase {
  ...
  static Future<RecessDatabase> open() async {
    ...
    final result = RecessDatabase(NativeDatabase.createInBackground(
  ...
  Future<String?> setting(String key) async {
    final rows = await customSelect('SELECT value FROM settings WHERE key = ?',
  ...
  Future<WorkSchedule?> schedule() async {
    ...
    return WorkSchedule(
        startMinutes: int.parse(start), endMinutes: int.parse(end));
  }
  ...
  Future<TodayProgress> todayProgress() async {
    ...
    final counts = <String, int>{
      for (final row in rows) row.read<String>('status'): row.read<int>('total')
    };
